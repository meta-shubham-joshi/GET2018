...
public class JobScheduler {
	...
	// Initialising the value of arrival time and burst time for each process
	...
	public int completionTime(JobScheduler jobs) {
		...
		jobs.completion_Time = jobs.arrival_Time + jobs.waiting_Time
				+ jobs.burst_Time;
	...
	public int waitTime(JobScheduler jobs, JobScheduler previousJob) {
		...
		if (jobs.arrival_Time < previousJob.last_Completed_Time) {
			...
			jobs.waiting_Time = previousJob.last_Completed_Time
					- jobs.arrival_Time;
			...
			jobs.waiting_Time = 0;
			jobs.last_Completed_Time = previousJob.last_Completed_Time
					+ (jobs.arrival_Time - previousJob.last_Completed_Time)
					+ jobs.burst_Time;
		}
	...
	// Calculating average waiting time for the process
	...
	public static void main(String args[]) {
		...
		for (int i = 0; i < pro; i++) {
			...
			System.out
					.println("Enter the values of arrival time for the process");
			arr[i][0] = sc.nextInt();
			System.out
					.println("Enter the values of burst time for the process");
			arr[i][1] = sc.nextInt();
		}
