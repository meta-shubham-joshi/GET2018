...
public class GraphImplementation implements Graph {
	...
	private void calculateShortestDistances (GraphStructure graph,int startVertex,int destinationVertex) {
		...
		if(pathList.contains(destinationVertex)) {
			...
			if(startVertex==destinationVertex) {
				...
				while(! flag) {
					int value = getVertexWithMinimumDistanceValue(list);
					if(! isVisited[value]) {
						isVisited[value]=true;
						shortestPathList.add(value);
						calculateShortestDistances(graph,value, destinationVertex);
					} 
				}
			}
		} else {
	...
	private int getVertexWithMinimumDistanceValue(List<Edge> list) {
		int min=distanceArray[1];
		int returningIndex=1;
		//Edge edge;
		for (Edge value : list) {
			if(! isVisited[value.getVertex()]) {
				if (min>distanceArray[value.getVertex()]) {
					min=distanceArray[value.getVertex()];
					returningIndex = value.getVertex();
				}
			}
		}
