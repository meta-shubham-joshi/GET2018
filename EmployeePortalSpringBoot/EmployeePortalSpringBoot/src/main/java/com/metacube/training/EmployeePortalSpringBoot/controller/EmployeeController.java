...
public class EmployeeController {
	...
	public String search(Model model, HttpServletRequest request) {
		model.addAttribute(
				"employee",
				employeeService.getEmployeeById(request.getSession()
						.getAttribute("employeeCode").toString()));
	...
	public String forgotPassword(
			...
			@RequestParam("newPassword") String newPassword) {
		Employee employee = employeeService.getEmployeeById(employeeCode);
		if ((employee != null) && (employee.getDob().equals(dob))) {
			employee.setPassword(newPassword);
			System.out.println("Password setted:::::" + employee.getPassword()
					+ ":::for Id:::::::" + employee.getEmpCode());
			employeeService.updateEmployee(employee);
		}
		return "redirect:/employee/login";
	}
