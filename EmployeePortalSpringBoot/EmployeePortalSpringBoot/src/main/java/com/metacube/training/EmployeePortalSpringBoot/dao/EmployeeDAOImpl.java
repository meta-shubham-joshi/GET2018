...
public class EmployeeDAOImpl implements EmployeeDAO {
	...
	private final String SQL_UPDATE_EMPLOYEE_DETAILS = "update employee set password = ? where emp_code = ?";
	...
	public boolean createEmployee(Employee employee) {
		return jdbcTemplate.update(SQL_INSERT_Employee, employee.getEmpCode(),
				...
				employee.getPassword(), employee.getEnabled()) > 0;
	}
	...
	public Employee getEmployeeDetailsByEmpCode(String empCode) {
		return jdbcTemplate.queryForObject(SQL_FIND_EMPLOYEE,
				new Object[] { empCode }, new EmployeeMapper());
	}

	@Override
	public Employee validateCredentials(String emp_code) {
		return jdbcTemplate.queryForObject(SQL_VALIDATE_CREDENTIALS,
				new Object[] { emp_code }, new ValidateMapper());
	}

	@Override
	public boolean updateEmployeePasswordInDatabase(Employee employee) {
		return jdbcTemplate.update(SQL_UPDATE_EMPLOYEE_DETAILS,
				employee.getPassword(), employee.getEmpCode()) > 0;
	}
